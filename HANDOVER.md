...
- Popup visual rules:
  ...
  - `Total Cost` / `Total Profit` and `Credits Available` belong in fixed bottom summary rows above the footer buttons on trade/review screens, not inside the scrollable weapon list. The net value cell uses red fill when trades cost credits and green fill when trades produce profit; text stays normal white/default.
