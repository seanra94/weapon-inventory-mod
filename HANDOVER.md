...
- Popup purchase flow:
  ...
  - `StockPurchaseService` owns high-level buy/sell workflows. Shared cargo/item stack operations belong in `StockItemCargo`; best-effort `PlayerMarketTransaction` construction belongs in `StockMarketTransactionReporter`; market/source discovery and sell-target choice belong in `StockPurchaseMarketSources`; cheapest-source buy plan math belongs in `StockPurchasePlan`.
