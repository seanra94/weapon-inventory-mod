...
- Popup purchase flow:
  ...
  - `StockPurchaseService` owns high-level buy/sell workflows and source selection. Shared cargo/item stack operations belong in `StockItemCargo`; best-effort `PlayerMarketTransaction` construction belongs in `StockMarketTransactionReporter`; cheapest-source buy plan math belongs in the service's `PurchasePlan` helper.
