...
- Popup visual rules:
  ...
  - `Warning`, `Total Cost` / `Total Profit`, `Credits Available`, `Total Purchase Volume`, and `Cargo Space Available` belong in fixed bottom summary rows above the footer buttons on trade/review screens, not inside the scrollable weapon list. The warning row starts at `None` and otherwise displays the most recently triggered trade warning. The net value cell uses red fill when trades cost credits, green fill when trades produce profit, and orange fill when a positive cost is between 95% and 100% of available credits; text stays normal white/default.
